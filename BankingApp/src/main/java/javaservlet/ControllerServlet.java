	...
	private void forwardController(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		case("addnewuser"):{
			...
			try {
				if(userType.equalsIgnoreCase("customer")) {
					...
					admin.insertCustomerRecord(id,  request.getParameter("pancard"),Long.parseLong(request.getParameter("aadhaar")));
		...
		case("addnewaccount"):{
			try {
				...
				home.deposit(Integer.parseInt(request.getParameter("userId")),  accountNumber,Double.parseDouble(request.getParameter("balance")));
		...
		case("saveprofile"):{
			...
			try {
				if(((String)banking.getAttribute("type")).equals("customer")) {
